...
def scenario_a_mono() -> dict:
    now = [0]
    engines = {}
    p = StreamingPipeline(clock=lambda: now[0],
                          factory=lambda lang: engines.setdefault(lang, FakeEngine(lang)))
    engines["vi"].inject_partial("xin chào mọi người", 0.9)
    _run(p, now, 250, SPEECH)  # 5 s VI
    n = len(_partials(p))
    return {"partials_5s": n, "expected_min": 25,
            "ok": n >= 25, "utterances": p.metrics.utterances}
...
def scenario_b_inter_utterance() -> dict:
    ...
    for lang, text in (("vi", texts["vi"]), ("en", texts["en"]),
                       ("zh", texts["zh"])):
        current[0] = lang
        # Live partials on the active engine; mismatched language decodes
        # with low confidence (what a real transducer produces), which is
        # what lets the endpoint verification tell them apart.
        p.active_asr.inject_partial(text, 0.9 if lang == p.active_lang else 0.35)
    ...
    return {"finals": len(finals), "expected_finals": 3,
            "endpoint_switches": p.router.switch_count, "expected_switches": 2,
            "ok": len(finals) == 3 and p.router.switch_count == 2}
